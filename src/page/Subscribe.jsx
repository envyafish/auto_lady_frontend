...
const Subscribe = () => {
    ...
    const fetchRecommendCodes = () => {
        setCodes([])
        setLoading(true)
        Api.post('/codes/recommend').then(res => {
            setLoading(false)
            setCodes(res.data)
            setTotal(20)
        }).catch(e => {
                ...
                <div>
                    ...
                    {activeTab !== '猜你喜欢' &&
                        <label className="input input-bordered btn-block btn-sm flex items-center gap-2 mt-2">
                            <input type="text" className="grow" placeholder="番号、演员、类型、制作商、发行商"
                                   value={keyword}
                    ...
                    <div
                        ...
                        {codes.map((item, index) => (
                            <CodeCard code={item} key={item.code} onRefresh={fetchCodes}></CodeCard>
                        ))}
                    </div>
